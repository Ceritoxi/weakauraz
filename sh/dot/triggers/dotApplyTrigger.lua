...
local trigger = function(_, _, subevent, _, _, castername, _, _, destGUID, _, _, _, _, name)
    if ((subevent == "SPELL_AURA_APPLIED" or subevent == "SPELL_AURA_REFRESH") and name ==
        aura_env.dots[aura_env.config.dot].name and castername == UnitName("player")) then
...
local untrigger = function(_, _, subevent, _, _, castername, _, _, destGUID, _, _, _, _, name)
    if (subevent == "SPELL_AURA_REMOVED" and name == aura_env.dots[aura_env.config.dot].name and castername ==
